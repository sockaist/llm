...
- 🔍 Vector DB (PostgreSQL + pgvector) 기반 정보 검색
- 📚 전산학부 특화 정보 제공
...
# 선택: 기본값은 text-embedding-3-small (1536)
...
#### 방법 2: Uvicorn 직접 실행
```bash
uv run uvicorn backend.server.server:app --app-dir src --host 0.0.0.0 --port 8000 --reload
```
...
└── backend/
    ...
    ├── llm/
    │   ├── inputChecker.py        # 입력 유효성 검사
    │   ├── inputNormalizer.py     # 입력 정규화
    │   ├── openai_chatbot.py      # OpenAI 응답 생성
    │   ├── vector_searcher.py     # Vector DB 검색
    │   └── query_date_filter.py   # 날짜 필터 추출
    └── vector_db/
        ├── vector_db_helper.py    # pgvector 스키마/CRUD/검색
        ├── embedding.py           # OpenAI 임베딩
        └── vector_db_main.py      # 적재 실행 스크립트
```
...
### 2. Vector DB 연결 실패
→ PostgreSQL 서버/DB가 실행 중이고 `pgvector` extension이 활성화되어 있는지 확인
