...
async def prepare_comparison():
    print("🚀 Preparing Comparison Baseline...")
        ...
        for col in TARGET_COLS:
            try:
                ...
                try:
                    ...
                    count = info.vectors_count if info.vectors_count is not None else info.points_count
                except Exception:
                    count = 0
                ...
                if count < 20:
                    print(f"⚠️ [{col}] count low (or missing). Creating & Ingesting subset...")
    ...
    with acquire_manager() as mgr:
        for col in TARGET_COLS:
            ...
            for p in points:
                text = p.payload.get("content") or p.payload.get("contents") or p.payload.get("text")
    ...
    print(f"✅ Saved {len(final_queries)} queries to {out_path}")
