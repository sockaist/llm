...
async def profile_search_pipeline(mgr, query: str, col: str):
    ...
    if results:
        print(f"Top result parent context: {'Yes' if results[0].get('parent_context') else 'No'}")
...
async def run_comprehensive_analysis():
    ...
    with acquire_manager() as mgr:
        ...
        os.system("PYTHONPATH=$PYTHONPATH:$(pwd)/src python src/scripts/clear_caches.py")
