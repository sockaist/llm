...
async def repair_ids():
    ...
    with acquire_manager() as mgr:
        for i, q in enumerate(queries):
            ...
            is_research = "csweb.research" in q.get("origin_collection", "") or "name:" in q["query"]
            
            if not is_research:
                continue
                
            current_id = q.get("expected_doc_ids", [])[0] if q.get("expected_doc_ids") else None
            query_text = q["query"]
            ...
            if results:
                ...
                if found_id and found_id != current_id:
                    print(f"   [Fixed] Query: {query_text[:20]}... | Old: {current_id[:8]} -> New: {found_id[:8]}")
