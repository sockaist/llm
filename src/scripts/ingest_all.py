...
def ingest_all():
    ...
    for root, dirs, files in os.walk(DATA_ROOT):
        ...
        print(f"Triggering ingestion for '{rel_path}' -> '{collection_name}' ({len(json_files)} files)...")
        
        try:
            # 1. Ensure collection exists
            try:
                requests.post(f"{API_URL}/admin/collections/create", json={
            ...
            resp = requests.post(f"{API_URL}/batch/upsert", json={
                "folder": root,
