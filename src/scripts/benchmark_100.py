...
def calculate_metrics(results):
    if not results:
        return {"count": 0, "hit_at_1": 0, "hit_at_5": 0, "mrr": 0, "avg_latency": 0, "p95_latency": 0, "avg_conf": 0, "avg_recall": 0, "ndcg_at_5": 0}
...
async def run_benchmark_100(query_file="src/tests/test_queries.json"):
    print("🚀 Starting 300-Query Benchmark with Advanced Metrics...")
    ...
    with acquire_manager() as mgr:
        ...
        for i, q_item in enumerate(queries):
            ...
            try:
                ...
                res = []
                error_msg = str(e)
                errors.append({"query": query_text, "error": error_msg, "col": str(origin_info)})
            ...
            if res:
                top_score = res[0].get('confidence') if res[0].get('confidence') is not None else res[0].get('score', 0.0)
                ...
                for r_idx, r in enumerate(res):
                    rid = r.get('id')
                    pid = r.get('payload', {}).get('db_id') or r.get('payload', {}).get('id')
    ...
    f1_score = 2 * (avg_precision * total_metrics['avg_recall']) / (avg_precision + total_metrics['avg_recall']) if (avg_precision + total_metrics['avg_recall']) > 0 else 0.0
    ...
    print(f"✅ Hit@1:    {total_metrics['hit_at_1']*100:.1f}%")
    print(f"✅ Hit@5:    {total_metrics['hit_at_5']*100:.1f}%")
    print(f"📈 MRR:      {total_metrics['mrr']:.3f}")
    print(f"⭐ NDCG@5:   {total_metrics['ndcg_at_5']:.3f} (New!)")
    
    print("-" * 30)
    print(f"🎯 Precision: {avg_precision:.3f} (@10)")
    print(f"🔍 Recall:    {total_metrics['avg_recall']:.3f} (@10)")
    ...
    print(f"⚡ Latency:   {total_metrics['avg_latency']:.1f}ms (Avg)")
    ...
    for t, rows in type_results.items():
        ...
        f1 = 2 * (ap * m['avg_recall']) / (ap + m['avg_recall']) if (ap + m['avg_recall']) > 0 else 0.0
        print(f"{t:<10} | {m['count']:<5} | {m['ndcg_at_5']:.3f} | {f1:.3f} | {m['avg_latency']:4.0f}ms")
    print("="*50)
