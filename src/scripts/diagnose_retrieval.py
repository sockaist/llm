...
async def diagnose_failures():
    ...
    print(f"[SEARCH] Diagnosing {len(research_queries)} research queries...")
    ...
    with acquire_manager() as mgr:
        for q in research_queries:
            ...
            if dense_res:
                ...
                print(
                    f"   [Dense] Top result: {top1_dbid[:8]}... (Score: {dense_res[0].score:.4f})"
                )
            ...
            if sparse_res:
                ...
                print(
                    f"   [BM25]  Top result: {top1_dbid[:8]}... (Score: {sparse_res[0].score:.4f})"
                )
            ...
            if splade_res:
                ...
                print(
                    f"   [SPLADE] Top result: {top1_dbid[:8]}... (Score: {splade_res[0].score:.4f})"
                )

            print(
                f"   => Ranks: Dense={dense_rank}, BM25={sparse_rank}, SPLADE={splade_rank}"
            )
    ...
    print(
        f"   - Dense : Hit Rate={dense_hits / total * 100:.1f}% | MRR={dense_rrs / total:.3f}"
    )
    print(
        f"   - BM25  : Hit Rate={sparse_hits / total * 100:.1f}% | MRR={sparse_rrs / total:.3f}"
    )
    print(
        f"   - SPLADE: Hit Rate={splade_hits / total * 100:.1f}% | MRR={splade_rrs / total:.3f}"
    )
