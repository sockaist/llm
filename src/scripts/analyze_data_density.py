...
def analyze_research_data():
    ...
    print(f"   - Total: {total}")
    print(
        f"   - Empty/Short 'etc' (<10 chars): {empty_etc} ({empty_etc / total * 100:.1f}%)"
    )
    print(
        f"   - Short 'content' (<100 chars): {short_content} ({short_content / total * 100:.1f}%)"
    )
    print(
        f"   - Rich 'content' (>=100 chars): {rich_content} ({rich_content / total * 100:.1f}%)"
    )
