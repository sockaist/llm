    ...
    with acquire_manager() as mgr:
        ...
        try:
            scroll_res, _ = mgr.client.scroll(
                collection_name="csweb.research",
                limit=1000,
                with_payload=True
            )
            # Fix: Research docs might not have 'text', accept them anyway for now
            research_docs = scroll_res # [r for r in scroll_res if r.payload.get("text")]
        ...
        for q in queries:
            ...
            try:
                ...
                points, _ = mgr.client.scroll(
                    ...
                    scroll_filter=models.Filter(
                        must=[models.FieldCondition(key="db_id", match=models.MatchAny(any=target_ids))]
                    ),
                    with_payload=True,
            ...
            try:
                if col == "csweb.research" and research_docs:
                    ...
                    candidates = [d for d in research_docs if d.id not in target_ids and d.payload.get("db_id") not in target_ids]
                    if candidates:
