...
TEST_DATA = [
    {
        "id": "admin_1",
        "title": "교무처 학사팀",
        "content": "학사운영, 성적관리, 졸업심사 관련 업무를 담당합니다. 전화: 042-350-2345",
    },
    {
        "id": "admin_2",
        "title": "학생처 장학팀",
        "content": "장학금 지급, 국가장학금 신청, 학자금 대출 업무를 담당합니다. 전화: 042-350-2346",
    },
    {
        "id": "research_1",
        "title": "딥러닝 최신 연구",
        "content": "2024년 딥러닝은 고성능 트랜스포머 아키텍처와 SSM 기반의 효율적 모델링이 핵심입니다.",
    },
    {
        "id": "research_2",
        "title": "해양 기상 관측 기술",
        "content": "해양 기상 관측을 위한 자율 운항 선박 및 센서 네트워크 기술이 발전하고 있습니다.",
    },
]
...
async def run_benchmark():
    ...
    print(f"[INFO] Initializing Phase 5 Benchmark on '{col_name}'...")
    ...
    with acquire_manager() as mgr:
        ...
        for q_entry in QUERIES:
            ...
            if top_hit:
                print(
                    f"  [Debug] Query: {query} | Top Result ID: {top_hit.get('id')} | Payload ID: {top_hit.get('payload', {}).get('id')}"
                )
            ...
            for i, res in enumerate(search_res):
                ...
                if (
                    res.get("id") == expected
                    or res.get("payload", {}).get("id") == expected
                ):
                    rank = i + 1
                    break
        ...
        print("=" * 40)
        print(f"[OK] Hit Rate @ 1: {avg_hit * 100:.1f}%")
        print(f"📈 MRR: {avg_mrr:.3f}")
        print(f"[ASYNC] Avg Latency: {avg_latency:.1f}ms")
