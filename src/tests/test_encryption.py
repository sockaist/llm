...
try:
    ...
    def test_encryption_flow():
        ...
        filter_secret = models.Filter(
            must=[
                models.FieldCondition(
                    key="id", match=models.MatchValue(value="doc_secret")
                )
            ]
        )
        res, _ = manager.client.scroll(
            COLLECTION, scroll_filter=filter_secret, limit=1, with_payload=True
        )

        if not res:
            print("[FAIL] Secret doc not found in DB!")
            sys.exit(1)
        ...
        assert "blue_dolphin" not in stored_content, (
            "[FAIL] Content is NOT encrypted at rest!"
        )
        assert payload.get("content_encrypted") is True, (
            "[FAIL] content_encrypted flag missing/false"
        )
        print("[OK] Encryption at Rest Verified")
        ...
        next(
            (
                r
                for r in results
                if r["id"] == "doc_secret_id" or "doc_secret" in str(r)
            ),
            None,
        )
        ...
        if not found_text:
            ...
            print("[FAIL] Secret doc not returned by search pipeline (Ranker issue?)")
            print([r.get("metadata", {}).get("id") for r in results])
        else:
            print(f"Retrieved Text: {found_text}")
            assert "blue_dolphin" in found_text, "[FAIL] Auto-Decryption Failed!"
            print("[OK] Auto-Decryption Verified")
        ...
        if found_encrypted:
            ...
            assert "blue_dolphin" not in found_encrypted, (
                "[FAIL] Admin decrypted User's private doc! (SECURITY FAILURE)"
            )
            assert "gAAAA" in found_encrypted or len(found_encrypted) > 20, (
                "[FAIL] Content doesn't look encrypted"
            )
            print("[OK] Admin Blindness Verified")
        else:
            print(
                "Admin didn't find doc (maybe acceptable if filter not updated yet, but Admin should see public/all)"
            )
