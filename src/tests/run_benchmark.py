...
class DBAdapter:
    ...
    async def search(self, query: str, top_k: int = 10):
        ...
        return await asyncio.to_thread(self.mgr.query, query, top_k=top_k, collections=collections)

async def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--phase", required=True, help="Experiment phase name (e.g. baseline)")
    parser.add_argument("--queries", default="src/tests/test_queries.json", help="Path to test queries")
    parser.add_argument("--num_queries", type=int, default=20, help="Number of queries to run")
    ...
    if not benchmark.test_queries:
        print("❌ No test queries loaded. Please run generate_test_data.py first.")
        return
    ...
    with acquire_manager() as mgr:
        print("✅ VectorDB Manager acquired.")
