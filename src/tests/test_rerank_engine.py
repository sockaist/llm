import torch
from llm_backend.vectorstore import rerank_engine
...
def test_rerank_engine_with_stub(monkeypatch):
    # Mock _load_model to return a FakeCrossEncoder
    monkeypatch.setattr(rerank_engine, "_load_model", lambda name: _FakeCrossEncoder())
    ...
    reranked, timing = rerank_engine.rerank(
        query="q",
        docs=docs,
