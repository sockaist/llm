    ...
    with acquire_manager() as mgr:
        ...
        for i, r in enumerate(results, 1):
            ...
            if db_id == expected_id:
                found = True
            print(f"{i:2d}. {db_id[:12]}... | Score: {score:.4f} | Avg: {avg_score:.4f} | {title}{match}")
            
        if not found:
            print("\n❌ Expected ID NOT FOUND in top 20.")
            ...
            hits, _ = mgr.client.scroll(
                ...
                scroll_filter=Filter(must=[FieldCondition(key="db_id", match=MatchValue(value=expected_id))]),
                limit=1,
                with_payload=True
            )
            if hits:
                print("✅ Found expected ID in DB!")
                print("Payload Preview:", hits[0].payload.get("title"))
            else:
                print("❌ Expected ID NOT FOUND in DB collection 'csweb.ai'.")
