...
def test_upsert_enforcement(mock_acquire):
    ...
    client.post(
        "/crud/upsert", data={"collection": "test"}, files=files, headers=USER_HEADERS
    )
    ...
    files = {"file": ("doc.json", file_content, "application/json")}
    client.post(
        "/crud/upsert", data={"collection": "test"}, files=files, headers=ADMIN_HEADERS
    )

    args, kwargs = mock_mgr.upsert_document.call_args
    data_arg = args[1]
    assert data_arg["tenant_id"] == "public", (
        "Admin upsert should allow public tenant_id"
    )
