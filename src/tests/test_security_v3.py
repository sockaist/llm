...
from llm_backend.server.vector_server.core.security.audit_logger import ProductionAuditLogger
...
class TestMetadataValidation:
    def test_reserved_fields(self):
        # Valid input
        valid, msg = MetadataValidator.validate_input({"title": "Doc A", "author": "bob"})
        assert valid is True
        
        # Invalid input (Top level)
        invalid_1, msg = MetadataValidator.validate_input({"tenant_id": "other", "vector": []})
        ...
        invalid_2, msg = MetadataValidator.validate_input({"vector": [], "metadata": {"role": "admin"}})
...
class TestLogChain:
    ...
    def test_hash_chaining(self):
        ...
        try:
            ...
            computed_hash1 = hashlib.sha256((prev_hash1 + serialized).encode("utf-8")).hexdigest()
            ...
            computed_hash2 = hashlib.sha256((prev_hash2 + serialized2).encode("utf-8")).hexdigest()
