...
class PerformanceBenchmark:
    ...
    async def run_throughput_benchmark(self, vector_db, duration_sec: int = 60) -> float:
        """
        처리량 측정 (QPS)
    ...
    async def run_quality_benchmark(self, vector_db) -> Dict:
        ...
        """
        if not self.test_queries:
            return {"recall_at_10": 0, "precision_at_10": 0, "ndcg_at_10": 0, "avg_confidence": 0.0}
        ...
        for test_case in self.test_queries:
            ...
            if results:
                ...
                result_ids = [r.id if hasattr(r, 'id') else r.get('id') for r in results]
            else:
    ...
    def compare_phases(self, baseline: str, optimized: str):
        """두 phase 간 성능 비교"""
        try:
            baseline_result = next(r for r in self.results_history if r.phase == baseline)
            optimized_result = next(r for r in self.results_history if r.phase == optimized)
        except StopIteration:
            print("Comparison failed: Phase not found in history.")
            return
        
        print(f"\n📈 Performance Improvement: {baseline} → {optimized}")
        print(f"├─ Latency: {self._improvement(baseline_result.avg_latency_ms, optimized_result.avg_latency_ms)}")
        print(f"├─ Throughput: {self._improvement(baseline_result.qps, optimized_result.qps, inverse=True)}")
        print(f"├─ Memory: {self._improvement(baseline_result.memory_mb, optimized_result.memory_mb)}")
        print(f"└─ Recall: {self._improvement(baseline_result.recall_at_10, optimized_result.recall_at_10, inverse=True)}\n")
    ...
    def _calculate_ndcg(result_ids: List[str], expected_ids: set, k: int) -> float:
        ...
        dcg = sum([1 / np.log2(i + 2) for i, doc_id in enumerate(result_ids[:k]) if doc_id in expected_ids])
