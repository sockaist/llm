...
def manager():
    ...
    docs = [
        {"id": "doc_public", "content": "This is public information.", "tenant_id": "public", "access_level": 1},
        {"id": "doc_user_a", "content": "This is User A's private note.", "tenant_id": "user_a", "access_level": 1},
        {"id": "doc_user_b", "content": "This is User B's private note.", "tenant_id": "user_b", "access_level": 1},
        {"id": "doc_admin", "content": "This is Top Secret Admin data.", "tenant_id": "public", "access_level": 3},
    ]
...
def test_multitenancy_scenarios(manager):
    ...
    print("\n--- Test 1: Guest Access ---")
    results = run_query_pipeline(manager, query, top_k=10, collections=[COLLECTION], cfg=cfg, user_context=None)
    ...
    results = run_query_pipeline(manager, query, top_k=10, collections=[COLLECTION], cfg=cfg, user_context=ctx_a)
    found_ids = [r.get("payload", {}).get("id") for r in results]
    ...
    assert "doc_user_b" not in found_ids
    assert "doc_admin" not in found_ids
    print("✅ User A Test Passed")

    print("\n--- Test 3: Admin Access ---")
    ctx_admin = {"user_id": "admin_user", "role": "admin"}
    results = run_query_pipeline(manager, query, top_k=10, collections=[COLLECTION], cfg=cfg, user_context=ctx_admin)
    found_ids = [r.get("payload", {}).get("id") for r in results]
    ...
    assert "doc_admin" in found_ids # Admin doc is public tenant? No, doc_admin tenant="public". Correct.
    print("✅ Admin Test Passed")

    print("\n--- Test 4: Semantic Cache Isolation ---")
    # 1. User A searches
    print("User A searching 'cache_test'...")
    run_query_pipeline(manager, "cache_test", top_k=5, collections=[COLLECTION], cfg=cfg, user_context=ctx_a)
    ...
    res_a = run_query_pipeline(manager, "User A", top_k=5, collections=[COLLECTION], cfg=cfg, user_context=ctx_a)
    found_ids_a = [r.get("payload", {}).get("id") for r in res_a]
    assert "doc_user_a" in found_ids_a
    
    print("User B searching 'User A' (Should yield public docs only, NOT User A's doc from cache)")
    ctx_b = {"user_id": "user_b", "role": "user"}
    res_b = run_query_pipeline(manager, "User A", top_k=5, collections=[COLLECTION], cfg=cfg, user_context=ctx_b)
