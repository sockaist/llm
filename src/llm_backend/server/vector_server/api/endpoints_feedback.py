...
class FeedbackRequest(BaseModel):
    ...
    target_id: Optional[str] = Field(
        None, description="ID of the document interacted with"
    )
    path_type: str = Field(
        "recommendation", description="Source path: primary or recommendation"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional context (dwell_time, user_id)"
    )

    # Optional fields for learning
    field: Optional[str] = Field(
        None, description="Target field for rule update (e.g., category)"
    )
...
async def submit_feedback(req: FeedbackRequest):
    ...
    try:
        ...
        user_action = {
            "explicit": "positive"
            if req.action_type in ["like", "click"]
            else "negative",
        ...
        if not is_valid:
            logger.warning(
                f"[API:/feedback] Feedback rejected ({conf}) for '{req.query}'"
            )
            return FeedbackResponse(
                status="ignored",
                message="Low quality feedback rejected",
        ...
        return FeedbackResponse(
            status="success", message="Feedback processed", validator_confidence=conf
        )
