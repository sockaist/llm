...
router = APIRouter(prefix="/logs", tags=["Logs & Audit"], dependencies=[Depends(verify_api_key)])

# Re-use the admin secret check logic or import it if shared
# For now, let's implement a quick check or move the admin check to core.auth
    ...
    if not os.path.exists(log_file):
        ...
        try:
            ...
            if candidates:
                ...
                return LogResponse(lines=["No log file found."], total_read=0, has_more=False)
        except Exception:
             return LogResponse(lines=["Log directory inaccessible."], total_read=0, has_more=False)
...
async def get_audit_logs(
    ...
    return await get_system_logs(lines=lines, keyword="[Audit]", x_admin_secret=x_admin_secret)
