...
from llm_backend.server.vector_server.manager.snapshot_manager import (
    create_snapshot, list_snapshots, delete_snapshot, restore_snapshot
)
from llm_backend.server.vector_server.core.cache_manager import get_cache, bump_collection_epoch
...
from llm_backend.server.vector_server.models.response_models import (
    SnapshotResponse, BatchResponse, CollectionListResponse
)
...
_ADMIN_SECRET_HASH = hashlib.sha256(_ADMIN_SECRET_RAW.encode()).hexdigest() if _ADMIN_SECRET_RAW else None
...
async def create_collection_api(
    ...
    try:
        ...
        return SnapshotResponse(status="success", message=f"Collection '{req.name}' created")
    except Exception as e:
...
async def delete_collection_api(
    ...
    try:
        ...
        bump_collection_epoch(req.name)
        return SnapshotResponse(status="deleted", message=f"Collection '{req.name}' deleted")
    except Exception as e:
...
async def retrain_bm25_api(req: BM25RetrainRequest = BM25RetrainRequest()):
    ...
    if os.getenv("ALLOW_BM25_BATCH", "1") != "1":
        raise HTTPException(status_code=403, detail="BM25 batch trigger is disabled by config")
    if is_job_active("bm25_retrain"):
        return BatchResponse(status="skipped", message="A BM25 job is already queued or running")
...
@router.get("/jobs/list", response_model=BatchResponse, response_model_exclude_none=True)
