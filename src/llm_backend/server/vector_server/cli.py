...
def cmd_init(args):
    ...
    print(f"\n[INFO] Initialized {CONFIG_FILE} with profile '{profile_name}'")
...
def cmd_check(args):
    ...
    try:
        ...
        if critical_count > 0:
            print(
                f"\n[FAIL] FAILED: {critical_count} blocking issues found. Deployment prevented."
            )
            sys.exit(1)
        else:
            print("\n[WARN] Passed with warnings (Non-blocking).")

    except Exception as e:
        print(f"[FAIL] Error parsing config: {e}")
        sys.exit(1)


def cmd_upgrade(args):
    print(
        "Upgrade wizard not implemented yet. Please modify vectordb.yaml manually to change tiers."
    )
