...
class SensitiveDataFilter:
    ...
    SENSITIVE_KEYS = {
        "ssn", "password", "passwd", "secret", "token", 
        "api_key", "credit_card", "cc_number", "cvv",
        "pin", "auth_token", "jwt", "access_token",
        "refresh_token", "private_key", "internal_id"
    }

    # Conditional PII keys (Blocked for non-admins unless explicitly allowed)
    PII_KEYS = {
        "email", "phone", "phone_number", "mobile", 
        "address", "dob", "birthdate", "salary"
    }

    @classmethod
    def scrub_data(cls, data: Union[Dict, List], user_role: str = "guest") -> Union[Dict, List]:
        """
    ...
    @classmethod
    def filter_search_results(cls, results: List[Dict], user_context: Dict) -> List[Dict]:
        """
...
class MetadataValidator:
    ...
    """
    RESERVED_FIELDS = {
        "role", "permissions", "is_admin", "admin", 
        "verified", "tier", "tenant_id", "_internal",
        "auth_level", "system_tags"
    }
