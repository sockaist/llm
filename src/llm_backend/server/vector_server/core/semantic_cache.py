...
class SemanticCache:
    ...
    @staticmethod
    def get(client, query_vector: List[float], query_text: str, threshold: float = 0.95, 
            user_context: Optional[Dict[str, Any]] = None) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            if search_result:
                hit = search_result[0]
                logger.debug(f"[SemanticCache] Hit! Score: {hit.score:.4f} for '{query_text}'")
                payload = hit.payload
    ...
    @staticmethod
    def set(client, query_vector: List[float], results: List[Dict[str, Any]], query_text: str, 
            user_context: Optional[Dict[str, Any]] = None, ttl: int = 3600):
        """
        ...
        try:
            ...
            # Hash of (query_text + user_id)
            import hashlib
            doc_id = hashlib.sha256((query_text + user_id).encode()).hexdigest()
