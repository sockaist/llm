...
def enqueue_job(job_type: str, payload: Dict[str, Any]) -> str:
    ...
    with _get_connection() as conn:
        conn.execute(
            """INSERT INTO jobs (id, type, payload, status, message, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (job_id, job_type, json.dumps(payload), "queued", "Waiting for Celery Worker", now, now)
        )
        conn.commit()
    ...
    try:
        ...
        with _get_connection() as conn:
             conn.execute("UPDATE jobs SET status='failed', message=? WHERE id=?", (f"Dispatch Error: {e}", job_id))
             conn.commit()
        raise e
...
def start_worker():
    logger.info("[QueueWorker] Running in Distributed Mode (Phase 2). Internal worker thread is DISABLED.")
    logger.info("[QueueWorker] Please ensure you are running 'celery -A llm_backend.server.vector_server.worker.celery_app worker --loglevel=info'")
...
def get_job_status(job_id: str) -> Dict[str, Any]:
    with _get_connection() as conn:
        cursor = conn.execute("SELECT id, type, status, message, created_at, updated_at FROM jobs WHERE id = ?", (job_id,))
        row = cursor.fetchone()
        if row:
            return dict(row)
...
def list_jobs(limit: int = 10) -> Dict[str, Any]:
    with _get_connection() as conn:
        cursor = conn.execute("SELECT id, type, status, message, created_at, updated_at FROM jobs ORDER BY created_at DESC LIMIT ?", (limit,))
