...
class MetadataRouter:
    ...
    def extract_filters(cls, query: str) -> Optional[models.Filter]:
        ...
        if year_match:
            ...
            must_conditions.append(models.FieldCondition(key="year", match=models.MatchValue(value=year)))
        ...
        if dept_res:
            dept, conf = dept_res
            must_conditions.append(models.FieldCondition(key="department", match=models.MatchValue(value=dept)))
        ...
        if cat_res:
            cat, conf = cat_res
            must_conditions.append(models.FieldCondition(key="category", match=models.MatchValue(value=cat)))
