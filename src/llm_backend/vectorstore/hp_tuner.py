...
def get_optimized_hp(query_text: str, current_cfg: dict) -> dict:
    ...
    if any(kw in query_lower for kw in ["전산학부", "학사", "졸업", "이수", "교과목", "수강", "학위", "학번", "장학", "총장"]):
        hp["title_weight"] = 0.8
        hp["dense_weight"] = 0.2
    ...
    elif any(kw in query_lower for kw in ["학과", "연락처", "교수", "누구", "전화", "이메일", "성함", "오시는"]):
        hp["title_weight"] = 0.7
        hp["dense_weight"] = 0.2
        hp["search_k"] = 30
    ...
    elif any(kw in query_lower for kw in ["동향", "설명", "알려줘", "무엇", "기술", "연구", "논문", "발표"]):
