...
class DynamicRuleDatabase:
    ...
    def load(self):
        ...
        if not os.path.exists(self.db_path):
            logger.warning(f"[DynamicDB] File not found: {self.db_path}. Using empty rules.")
    ...
    def save(self):
        ...
        try:
            ...
            backups = sorted([f for f in os.listdir(self.BACKUP_DIR) if f.startswith("rules_backup_")])
            if len(backups) > 10:
                for b in backups[:-10]:
                    os.remove(os.path.join(self.BACKUP_DIR, b))
                    
            logger.info(f"[DynamicDB] Atomic save successful. Backup created: {backup_name}")
        except Exception as e:
            logger.error(f"[DynamicDB] Error saving rules: {e}")

    def add_feedback(self, field: str, query: str, value: Any, feedback_score: float = 0.8, validator_conf: str = "high_confidence"):
        """
        ...
        if update_weight == 0.0:
            logger.warning("[DynamicDB] Feedback ignored due to low validator confidence.")
            return
        ...
        logger.info(f"[DynamicDB] Confidence updated for '{best_keyword}': {current_conf:.2f} -> {rule['confidence']:.2f}")
        self.save()
