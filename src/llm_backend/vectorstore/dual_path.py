...
class DualPathOrchestrator:
    ...
    def process_query(
        ...
        """
        logger.info(
            f"[DualPath] Processing query: '{query_text}' (User: {user_context.get('user_id') if user_context else 'None'})"
        )
        ...
        if query_filters:
            [
                ...
                if r.get("score_breakdown", {}).get("is_filtered", False)
            ]
            for r in semantic_results:
                r["path_type"] = "primary"
        else:
        ...
        for r in semantic_results:
            ...
            hard_score = (
                0.95 if query_filters else 0.5
            )  # Assumed high confidence if metadata matched
            ...
            if query_filters:
                ...
                tuned_conf = (0.5 * 0.4) + (
                    vector_score * 0.6
                )  # Blending with neutral hard score
        ...
        return {
            ...
            "debug": {"filters": str(query_filters) if query_filters else None},
        }
    ...
    def apply_feedback(
        ...
        logger.info(
            f"[DualPath] Applying {feedback_type} feedback ({validator_conf}) for '{query}' -> {path_type}"
        )
