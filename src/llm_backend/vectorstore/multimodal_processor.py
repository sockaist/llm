...
class MultimodalProcessor:
    ...
    async def index_pdf_images(self, client: QdrantClient, col_name: str, pdf_path: str):
        ...
        for i, img_data in enumerate(images_data):
            ...
            points.append(PointStruct(
                id=point_id,
                vector={"image": img_emb},
                payload=payload
            ))

        if points:
            client.upsert(collection_name=col_name, points=points)
            logger.info(f"Indexed {len(points)} images from {pdf_path} into '{col_name}'")

    async def search_images_by_text(self, client: QdrantClient, col_name: str, query_text: str, top_k=5):
