...
class LLMChain(Chain):
    ...
    def __init__(
        self, chatbot: ChatBot, prompt: InstructionConfig, output_key: str = "output"
    ):
        """
        LLMChain 초기화
...
class RouterChain(Chain):
    ...
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if destination_key in self.destination_chains:
            ...
            raise ValueError(
                f"라우팅 대상 '{destination_key}'에 해당하는 체인이 없고 기본 체인도 설정되지 않았습니다."
            )
    ...
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for var in self.output_variables:
            if var not in outputs:
                raise ValueError(
                    f"변환 함수가 필요한 출력 변수 '{var}'를 생성하지 않았습니다."
                )
