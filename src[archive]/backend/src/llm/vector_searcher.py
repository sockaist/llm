...
try:
    ...
    try:
        ...
        def search_doc(client, query, col_name, k):
            ...
            q_sparse = bm25_encode(query)
            q_splade = splade_encode(query)

            results = client.search_batch(
                collection_name=col_name,
                requests=[
                    SearchRequest(vector=NamedVector(name="dense", vector=q_dense), limit=k),
                    SearchRequest(vector=NamedVector(name="sparse", vector=q_sparse), limit=k),
                    SearchRequest(vector=NamedVector(name="splade", vector=q_splade), limit=k)
                ]
            )

            res_dense, res_sparse, res_splade = results
            S_dense   = {r.id: r.score for r in res_dense}
            S_sparse  = {r.id: r.score for r in res_sparse}
            S_splade  = {r.id: r.score for r in res_splade}
            ...
            Sd, Ss, Sp = minmax(S_dense), minmax(S_sparse), minmax(S_splade)

            w_dense, w_splade, w_sparse = 0.5, 0.15, 0.35  # 네가 말한 비율
            combined = {}

            all_ids = set(Sd) | set(Ss) | set(Sp)
            for pid in all_ids:
                combined[pid] = (
                    w_dense * Sd.get(pid, 0)
                    + w_splade * Sp.get(pid, 0)
                    + w_sparse * Ss.get(pid, 0)
                )

            ranked = sorted(combined.items(), key=lambda x: x[1], reverse=True)
            top_k = [pid for pid, _ in ranked[:k]]

            # retrieve로 실제 payload 반환
            docs = client.retrieve(collection_name=col_name, ids=top_k, with_payload=True)
            return docs
    except ImportError:
